...
noded* add(noded* head, int data){
    if(!head) return create_noded(data);

    noded* temp = head;
    while(temp->next) temp = temp->next;
    noded* newnode = create_noded(data);
    temp->next = newnode;
    newnode->prev = temp;
    return head;
}
noded* to_doubly(node* head){
    if(!head) return NULL;

    noded* dhead = NULL;
    while (head)
    {
        dhead = add(dhead,head->data);
        head = head->next;
    }
...
void print_d_reversed(noded* head){
    if(!head){
        printf("List empty \n");
        return;
    }
...
int main(int argc, char const *argv[])
{
    ...
    head = break_circle(head);
    print_ll(head);

    noded * dhead = to_doubly(head);
    print_d(dhead);
    print_d_reversed(dhead);
    return 0;
}
